...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            # bbox 정보를 객체별로 정리 (방향 감지용)
            bboxes_dict = {}
            if len(det):
                for *xyxy, conf, cls in det:
                    class_name = names[int(cls)]
                    if class_name not in bboxes_dict:
                        bboxes_dict[class_name] = []
                    # xyxy는 tensor이므로 리스트로 변환
                    bbox = [coord.item() if hasattr(coord, 'item') else coord for coord in xyxy]
                    bboxes_dict[class_name].append(bbox)
            
            cd.add(names, detected, save_dir, im0, bboxes=bboxes_dict if bboxes_dict else None)  #added by kiokahn, 20220608
