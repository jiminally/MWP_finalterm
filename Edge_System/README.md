...
### MediaPipe Installation (Required for Counter Mode)

This project uses **MediaPipe 0.10.x** for hand gesture detection in `counter` mode. The latest MediaPipe versions (0.11+) use a different API structure, so you must install version 0.10.x.

```bash
# Install MediaPipe 0.10.x (compatible version)
pip install mediapipe==0.10.13

# Or install the latest 0.10.x version available
pip install "mediapipe>=0.10.13,<0.11.0"
```

**Important Notes:**
- MediaPipe is only required for `counter` mode (hand gesture detection)
- `entrance_outside` and `entrance_inside` modes work without MediaPipe
- If you encounter import errors, uninstall and reinstall:
  ```bash
  pip uninstall mediapipe -y
  pip install mediapipe==0.10.13
  ```
